...
import { optimizeUploadBuffer, registerImageRoutes } from "./server/imageOptimizer";
import {
...
// Gzip every text response. JSON content payloads compress by roughly 4-5x, and the
// app was previously shipping all of them uncompressed.
app.use(compression({
  filter: (req, res) => {
    // Images out of /api/img are already compressed formats — re-compressing burns CPU
    // for nothing.
    if (String(res.getHeader("Content-Type") || "").startsWith("image/")) return false;
...
// Page-scoped reads sit behind a short in-process cache. Any successful write has to
// drop that cache, so rather than sprinkling invalidation through ~30 CRUD handlers we
// hang it off the response of every mutating /api/ request.
const contentService = createContentService(supabase);

app.use((req, res, next) => {
  if (req.method !== "GET" && req.method !== "HEAD" && req.path.startsWith("/api/")) {
    res.on("finish", () => {
      if (res.statusCode < 400) invalidateContentCache();
    });
  }
  next();
});

// On-demand WebP conversion for images already in the database (/api/img).
registerImageRoutes(app);

// Serve static uploaded materials (Keep for backward compatibility during migration)
// Upload filenames carry a timestamp, so a given URL never changes contents.
...
app.post("/api/upload", async (req, res) => {
  ...
  try {
    ...
    // Compress and convert to WebP before it ever reaches storage. Vectors and animated
    // images pass through untouched.
    const optimized = await optimizeUploadBuffer(originalBuffer, filename);
    const uniqueFilename = optimized.filename;

    const { data, error } = await supabase.storage
      .from("uploads")
      .upload(uniqueFilename, optimized.buffer, {
    ...
    const savedPct = optimized.converted
      ? ` (${Math.round((1 - optimized.buffer.byteLength / originalBuffer.byteLength) * 100)}% smaller as WebP)`
      : "";
    console.log(`Successfully uploaded ${uniqueFilename} to Supabase${savedPct}.`);
...
//
// The site used to boot by downloading every collection and every configuration blob
// through /api/db, on every page. Public views now read /api/content?page=<name>, which
// returns just that view's slice; /api/db survives for the CMS, which really does edit
// everything at once.

/** Sends JSON with an ETag so repeat requests can be answered with a 304. */
function sendCachedJson(req: express.Request, res: express.Response, payload: any, cacheControl: string) {
...
async function handleContentRequest(req: express.Request, res: express.Response) {
  const page = String(req.params.page || req.query.page || "shell");

  if (!isContentPage(page)) {
    return res.status(400).json({
      error: `Unknown content page "${page}".`,
      availablePages: CONTENT_PAGES
    });
  }

  try {
    const payload = await contentService.getPage(page, {
      limit: req.query.limit ? Number(req.query.limit) : undefined,
      offset: req.query.offset ? Number(req.query.offset) : undefined,
      id: req.query.id ? String(req.query.id) : undefined
    });

    if (page === "activity" && !payload.article) {
      return res.status(404).json({ error: "Article not found." });
    }

    return sendCachedJson(req, res, payload, CONTENT_CACHE_CONTROL);
  } catch (err: any) {
    console.error(`Error building content page "${page}":`, err);
    return res.status(500).json({ error: "Failed to fetch page content" });
  }
}

app.get("/api/content", handleContentRequest);
app.get("/api/content/:page", handleContentRequest);

// Full payload — the CMS edits every collection, so it still needs all of it. Never
// cached: an admin has to see their own writes immediately.
app.get("/api/db", async (req, res) => {
  try {
    const db = await contentService.getFullDatabase();
...
async function startServer() {
  if (process.env.NODE_ENV !== "production") {
    ...
    // Everything else in dist (favicon, QR code, manifest) is stable but not fingerprinted.
    app.use(express.static(distPath, {
      maxAge: "1d",
      setHeaders: (res, filePath) => {
        // The HTML shell must always be revalidated or a deploy never reaches clients.
